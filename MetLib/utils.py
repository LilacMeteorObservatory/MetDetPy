...
class SlidingWindow(object):
    ...
    def __init__(self,
        ...
        if self.force_int and dtype == np.uint8:
            sum_dtype = np.uint32
        self.sum: Union[NDArray[np.float64],
                        NDArray[np.uint32]] = np.zeros(size, dtype=sum_dtype)
...
def estimate_snr_smooth_residual(image: U8Mat, kernel_size: int = 5) -> float:
    """
    估计单张图像的SNR（无参考图像情况下）。 generated by ChatGPT.
    
    使用平滑残差法：
    1. 高斯平滑 -> 近似无噪声图像
    2. 残差 = 原图 - 平滑图
    3. Var(平滑图) / Var(残差) = SNR in db.
    
    Args:
        image (U8Mat): input image
        kernel_size (int, optional): smooth kernel size. Defaults to 5.

    Returns:
        float: SNR(db).
    """
    if len(image.shape) == 3:  # 彩色图转灰度
