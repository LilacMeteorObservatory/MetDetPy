...
你可以从 [Release](https://github.com/LilacMeteorObservatory/MetDetPy/releases) 处获取最新的MetDetPy发行版。发行版将 MetDetPy 进行了打包，可独立在主流平台运行（Windows，macOS）。你也可以通过 `make_package.py` 选择 Nuitka 或 PyInstaller 后端自行构建独立可执行文件（见 [make_package](./tool-usage-cn.md#make_package)）。
