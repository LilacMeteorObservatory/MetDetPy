...
1. 正式定义流星检测结果格式(MDRF)作为MetDetPy的流转文件格式，增加各脚本的通用性。该格式文件可以手动创建（作为标注），从检测/评估工具导出，或作为评估和切片工具的输入。有关该脚本的更多介绍见()[]。`MetDetPy` 和 `MetDetPhoto` 可以输出MDRF文件，而`ClipToolkit` 和 `evaluate` 则可以接受MDRF文件的输入。

2. 重构并完善用于图像检测的工具入口 `MetDetPhoto`。该工具现可用于单张照片，延时序列或者延时序列构成的视频检测。

3. 为 `VideoLoader` 增加了 `debayer` 和 `debayer_pattern` 选项，所有和视频相关的功能现均支持对未解Bayer的视频/照片进行检测。

✅ Improvement(s)

1. 更新了的流星速度置信区间，改善对于卫星和飞机的误判。

2. 更新了检测模型 `yolov5s_v2.onnx` 作为默认新检测器，改善卫星线等场景的准确率，支持更多检测类别。

3. 调整了重校验阈值与最终预测的得分的计算方式。

4. 改进了 `ClipToolkit` 的易用性，主要有（1）提供了简化的调用接口，支持了无参数（完整视频）及单张截图时的参数；（2）允许同时保存Labelme格式标注；（3）支持对图像检测结果的输出；（4）导出视频时以流式保存，避免内存溢出。

5. 调整了后端模式下的输出，未被分类为正样本的检测结果也会在后端输出（同样以 Meteor 开头）。

6. 保存到检测结果文件的坐标与格式更改为与命令行输出一致（即视频原始分辨率下的坐标）。
