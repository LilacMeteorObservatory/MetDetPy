...
3. 检测性能
    1. 修复onnxruntime在windows平台暂时不支持GPU的问题，缓解CPU负载
    2. 简化迭代中产生的冗余检测后处理机制（如DyMask， MeteorLib中的冗余逻辑），提升检测性能。
    3. 构建直接使用差值代替线段检测的高速检测器，降低计算量，并预期能改善城市场景卫星/飞机的问题。
    4. 对噪声估计不准确的场景引入微调参数，改善该类型场景对暗弱流星的检测。
    6. 引入性能测试，确保迭代间的性能稳定性，量化性能迭代收益。
4. 已探明的问题修复
    1. 需要修复Stacker在特定场景下可能出现的问题，并增加兜底逻辑。
    2. 期望修复时间索引错位问题。
5. 其他杂项优化
